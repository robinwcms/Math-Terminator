...
// In timed mode the timer running out ends the challenge — use a different title
if (obj_game_manager.is_timed_mode)
{
	text_1 = "CHALLENGE ENDED";
}
else
{
	text_1 = "YOU LOSE";
...
// Sets the lose banner text to the players score (read from manager if player is gone)
...
{
	player_score = obj_game_manager.final_score;
}
...
high_score_buffer = buffer_load(global.hs_filename_for_mode());
...
buffer_save(high_score_buffer, global.hs_filename_for_mode());